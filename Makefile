...
.PHONY: all
all: fmt lint generate | $(BIN) ; $(info $(M) building executable…) @ ## Build program binary
	$Q $(GO) build \
		-tags release \
		-a -ldflags '-extldflags "-static" -X $(MODULE)/cmd.Version=$(VERSION) -X $(MODULE)/cmd.BuildDate=$(DATE)' \
...
.PHONY: generate
generate: ; $(info $(M) running bolierplate generation…) @ ## Run go:generate
	$Q $(GO) generate
